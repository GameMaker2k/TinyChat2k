...
if (!in_array($_GET['act'], $tinyactcheck)) {
    $_GET['type'] = "welcome";
}
if ($_GET['act'] == "check") {
    echo "{success:tinychat};";
    exit();
}
if (!isset($_GET['room'])) {
    $_GET['room'] = "";
...
if ($_GET['act'] == "status" && isset($_SESSION['userid']) && isset($_SESSION['username']) && isset($_GET['room'])) {
    ...
    if (!in_array($_GET['type'], $tinystatuscheck)) {
        $_GET['type'] = "welcome";
    }
    if ($_GET['type'] == "welcome") {
        echo "{timestamp:".get_microtime().",userid:".$welcomemsg['userid'].",username:\"".base64_encode($welcomemsg['username'])."\",message:\"".base64_encode(convert_message($welcomemsg['message']))."\"};";
        exit();
    }
    if ($_GET['type'] == "hello") {
        if ($_SESSION['msgstatus'] < 0) {
            echo "{error:message};";
            exit();
        }
        if ($_SESSION['msgstatus'] > 0) {
...
if ($_GET['act'] == "signup") {
    ...
    if ($numrows <= 0) {
        ...
        $findmember = sqlite3_query($sqlite_tinychat, "SELECT * FROM \"".$sqlprefix."members\" WHERE \"name\"='".sqlite3_escape_string($sqlite_tinychat, $_POST['username'])."';");
        $memberinfo = sqlite3_fetch_assoc($findmember);
        echo "{success:signupuser};";
        exit();
    }
    if ($numrows > 0) {
        echo "{error:signupuser};";
        exit();
    }
}
if ($_GET['act'] == "logout") {
    if (!isset($_SESSION['userid']) || !isset($_SESSION['username'])) {
        echo "{error:logoutuser};";
        exit();
    }
    if (isset($_SESSION['userid']) && isset($_SESSION['username'])) {
