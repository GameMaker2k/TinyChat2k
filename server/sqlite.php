...
/*
    ...
    $FileInfo: sqlite.php - Last Update: 01/01/2013 Ver. 0.0.1 - Author: cooldude2k $
*/
...
{
    $return_text = null;
    if (isset($_COOKIE[$matches[1]])) {
        $return_text = $_COOKIE[$matches[1]];
    }
    if (!isset($_COOKIE[$matches[1]])) {
        $return_text = null;
    }
...
function get_setting_values($matches)
{
    ...
    if (isset($Settings[$matches[1]])) {
        $return_text = $Settings[$matches[1]];
    }
    if (!isset($Settings[$matches[1]])) {
        $return_text = null;
    }
...
function convert_message($message)
{
    ...
    if (trim($LOG_QUERY_STRING, "\x00..\x1F") == "") {
        $LOG_QUERY_STRING = "";
    }
